...
class GraphFinetuneLearner(learner.BaselineLearner):
  ...
  def __init__(self,
    ...
    self.ancestors_filepath = ancestors_filepath
    with tf.io.gfile.GFile(self.ancestors_filepath) as f:
    ...
    for _, ancestors in sorted(ancestor_dict.items()):
      if len(ancestors) < self.max_ancestors:
        paddings = [[0, self.max_ancestors-len(ancestors)]]
        self.ancestor_lookup.append(tf.pad(ancestors[:self.max_ancestors], paddings, "REFLECT"))
      else:
        self.ancestor_lookup.append(ancestors[:self.max_ancestors])
    self.ancestor_lookup = tf.constant(self.ancestor_lookup, dtype=tf.int64)
  ...
  def compute_logits(self):
    ...
    if self.is_training:
      logits, graph_logits = self.forward_pass_fc(self.train_embeddings)
    else:
      # ------------------------ Finetuning -------------------------------
      ...
      logits, graph_logits = self._fc_layer(self.train_embeddings, self.data.train_labels, self.data.train_class_ids)
      finetune_loss = self._classification_loss(logits, graph_logits, self.data.train_labels,
                                                self.way)
      ...
      with tf.control_dependencies([finetune_op] + vars_to_finetune):
        ...
        with tf.control_dependencies([print_op]):
          self.test_logits, self.test_graph_logits = self._fc_layer(test_embeddings, self.data.test_labels, self.data.test_class_ids)
          logits = self.test_logits
          graph_logits = self.test_graph_logits
    return logits, graph_logits
  ...
  def graph_classifier_logits(self, embeddings, label_to_id, num_nodes, cosine_classifier,
    ...
    if use_weight_norm:
      ...
      # print_op = tf.print(['embeddings', embeddings, 'class_bias', class_bias, 'class_embeddings', class_embeddings])
      # with tf.control_dependencies([print_op]):
      logits = learner.linear_classifier_forward_pass(embeddings, class_embeddings, class_bias,
                                              cosine_classifier,
                                              cosine_logits_multiplier, False)

      graph_logits = learner.linear_classifier_forward_pass(embeddings, self.node_embeddings, self.node_bias,
                                            cosine_classifier,
                                            cosine_logits_multiplier, False)
    return logits, graph_logits
  ...
  def forward_pass_fc(self, embeddings):
    ...
    with tf.variable_scope('fc', reuse=tf.AUTO_REUSE):
      logits, graph_logits = self.graph_classifier_logits(embeddings,
  ...
  def _fc_layer(self, embedding, labels, classes):
    ...
    with tf.variable_scope('fc', reuse=tf.AUTO_REUSE):
      logits, graph_logits = self.graph_classifier_logits(embedding,
                                        label_to_id,
                                        self.num_nodes,
                                        self.cosine_classifier,
                                        self.cosine_logits_multiplier,
  ...
  def compute_loss(self):
    """Computes the loss."""
    if self.is_training:
      self.train_logits, self.train_graph_logits = self.compute_logits()
      labels = tf.cast(self.data.labels, tf.int64)
      ancestor_ids = tf.gather(self.ancestor_lookup, labels)
      ancestor_ids = tf.concat([tf.expand_dims(labels, -1), ancestor_ids], -1)

      onehot_labels = tf.one_hot(labels, self.num_train_classes)
      graph_labels = tf.reduce_sum(tf.one_hot(ancestor_ids, self.num_nodes, on_value=1 / (self.max_ancestors + 1)), 1)
      with tf.name_scope('loss'):
        class_loss = tf.losses.softmax_cross_entropy(
            onehot_labels=onehot_labels, logits=self.train_logits)
        class_loss = tf.reduce_mean(class_loss)

        graph_loss = tf.nn.softmax_cross_entropy_with_logits(labels=graph_labels, logits=self.train_graph_logits)
        graph_loss = tf.reduce_mean(graph_loss)

        regularization = tf.reduce_sum(
            tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES))
        loss = class_loss + graph_loss + self.weight_decay * regularization
        return loss
    else:
  ...
  def _classification_loss(self, logits, graph_logits, labels, num_classes):
    ...
    with tf.name_scope('finetuning_loss'):
      ...
      graph_loss = tf.nn.softmax_cross_entropy_with_logits(labels=graph_labels, logits=graph_logits)
      graph_loss = tf.reduce_mean(graph_loss)
